...
// 🔗 لینک تست
// ==========================================
const TEST_URL = "https://www.boyner.com.tr/joma-ace-men-25-erkek-acik-mavi-toprak-kort-tenis-ayakkabisi-taces2505c-p-15504791?srsltid=AfmBOor5bokGS2VXKHAYxJNGfCL9dFGZa8gf_k891Q1kbiuamaSMsFR1";
...
// ==========================================
async function scrapeBoyner(page, url) {
    console.log(`\n🔄 Scraping Boyner: ${url}`);
    try {
        if (useProxy) {
            await page.authenticate({ username: 'mehran', password: 'mehran75' });
        ...
        console.log(`  ✅ Boyner URL Scraped: ${Object.keys(normalizedStocks).length} sizes found.`);
        return { success: true, stocks: normalizedStocks, regular_price: regular, offer_price: offer };
...
async function scrapeProduct(page, url) {
    ...
    if (url.toLowerCase().includes('boyner.com.tr')) {
        return scrapeBoyner(page, url);
    }
