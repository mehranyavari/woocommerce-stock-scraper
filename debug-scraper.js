...
async function debug() {
    console.log('🐞 Starting Advanced Debug Scraper (NO PROXY + WITH STEALTH)...');
    console.log(`🎯 Target: ${TEST_URL}\n`);
    
    // ۱. اجرای مرورگر با پلاگین ضدردیابی اما بدون پروکسی
    const browser = await puppeteer.launch({
        headless: 'new',
        args: [
            '--no-sandbox', 
            '--disable-setuid-sandbox',
            '--disable-blink-features=AutomationControlled' // یک ترفند اضافه برای مخفی ماندن
        ]
    });
    ...
    try {
        ...
        // ۲. بریدن متن جیسون از داخل HTML
        ...
        if (!dktString) {
            ...
            console.log("📸 Taking a new screenshot to see if Stealth Plugin worked...");
